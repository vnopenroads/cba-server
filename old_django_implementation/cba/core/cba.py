...
class CostBenefitAnalysisModel:
    ...
    def compute_cba_for_section(self, section):
        ...
        results = {
            ...
            "iri_projection": dCondIRI[
                iTheSelected,
            ].tolist(),
            "iri_base": dCondIRI[
                0,
            ].tolist(),
            "con_projection": dCondCON[
                iTheSelected,
            ].tolist(),
            "con_base": dCondCON[
                0,
            ].tolist(),
            "financial_recurrent_cost": dCostRecurrentFin[
                iTheSelected,
            ].tolist(),
            "net_benefits": dNetTotal[
                iTheSelected,
            ].tolist(),
        }
        return results
    ...
    def compute_alternatives(self, iSurfaceType, iRoadClass, iConditionClass):
        ...
        idx = (iSurfaceType - 1) * 50 + (iRoadClass - 1) * 5 + iConditionClass - 1
        year, number, x, y = self.dWorkEvaluated[idx, :]
        # Get initial road work for the 13 alternatives
        dAlternatives[0, 0] = number
        dAlternatives[1, 0] = x
        dAlternatives[2, 0] = x
        dAlternatives[3, 0] = x
        dAlternatives[4, 0] = x
        ...
        dAlternatives[8, 0] = y
        dAlternatives[9, 0] = y
        dAlternatives[10, 0] = y
        dAlternatives[11, 0] = y
        dAlternatives[12, 0] = y

        # Define years of initial works for 13 alternatives
        dAlternatives[0, 1] = year
        ...
        if dAlternatives[1, 0] == 0 and dAlternatives[7, 0] == 0:  # no road works defined: evaluate 2 base alternatives
            dAlternatives[1, 0] = number
            dAlternatives[1, 1] = year
            iNoAlernatives = 2
